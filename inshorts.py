...
class Inshorts:
    ...
    def all_news(self,max_limits=5):
        ...
        try:
            ...
            var_list = ["author_name","content","source_url","source_name","title","category_names","image_url"]
            for j in range(max_limits):
                print('-'*30)
                for i in var_list:
                    print(f'{i} = {json_response["data"]["news_list"][j]["news_obj"][i]}')
    ...
    def category_news(self,category_name,max_limits=5):
        ...
        try:
            ...
            for j in range(max_limits):
                print('-'*30)
                for i in var_list:
                    print(f'{i} = {json_response["data"]["suggested_news"][j]["news_obj"][i]}')
        except Exception as e:
            print(f"Exception occured due to {e.args}")